"use client";

import { useCallback, useEffect, useState } from "react";
import { apiClient, ApiClientError } from "@/lib/api-client";
import { useAdminFilter } from "@/hooks/use-admin-filter";
import { useAuthActions } from "@/hooks/use-auth";
import { activeUnitId, unitQuery } from "@/lib/unit-scope";
import type {
  AgroMarketRow,
  KpiDefinition,
  KpiRecord,
  ProjectDetail,
  ProjectRow,
  RepresentativeRow,
} from "@/lib/module-types";

function errorMessage(err: unknown): string {
  if (err instanceof ApiClientError) return err.message;
  if (err instanceof TypeError && err.message === "Failed to fetch") {
    return "API gateway unreachable. Run .\\deploy\\scripts\\docker-up.ps1 and open http://localhost:3000";
  }
  if (err instanceof Error) return err.message;
  return "Request failed";
}

async function fetchWithRetry<T>(fn: () => Promise<T>): Promise<T> {
  try {
    return await fn();
  } catch (err) {
    const retryable =
      err instanceof TypeError ||
      (err instanceof ApiClientError && err.status >= 502);
    if (!retryable) throw err;
    await new Promise((r) => setTimeout(r, 600));
    return fn();
  }
}

function useModuleLoader() {
  const { isLoading: authLoading, isAuthenticated } = useAuthActions();
  const ready = !authLoading && isAuthenticated;
  return { ready };
}

export function useProjectsList() {
  const { filter } = useAdminFilter();
  const { ready } = useModuleLoader();
  const [rows, setRows] = useState<ProjectRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!ready) return;
    setLoading(true);
    setError(null);
    try {
      const json = await fetchWithRetry(() =>
        apiClient<{ success: boolean; data: ProjectRow[] }>(
          `projects${unitQuery(filter)}`,
        ),
      );
      setRows(json.data ?? []);
    } catch (err) {
      setRows([]);
      setError(errorMessage(err));
    } finally {
      setLoading(false);
    }
  }, [filter, ready]);

  useEffect(() => {
    void load();
  }, [load]);

  return { rows, loading: loading || !ready, error, reload: load };
}

export function useProjectDetail(projectId: string | null) {
  const { ready } = useModuleLoader();
  const [detail, setDetail] = useState<ProjectDetail | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!projectId) {
      setDetail(null);
      return;
    }
    if (!ready) return;
    setLoading(true);
    setError(null);
    try {
      const json = await fetchWithRetry(() =>
        apiClient<{ success: boolean; data: ProjectDetail }>(
          `projects/${projectId}`,
        ),
      );
      setDetail(json.data ?? null);
    } catch (err) {
      setDetail(null);
      setError(errorMessage(err));
    } finally {
      setLoading(false);
    }
  }, [projectId, ready]);

  useEffect(() => {
    void load();
  }, [load]);

  return { detail, loading, error, reload: load };
}

export function useRepresentativesList() {
  const { filter } = useAdminFilter();
  const { ready } = useModuleLoader();
  const [rows, setRows] = useState<RepresentativeRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!ready) return;
    setLoading(true);
    setError(null);
    try {
      const json = await fetchWithRetry(() =>
        apiClient<{ success: boolean; data: RepresentativeRow[] }>(
          `representatives${unitQuery(filter)}`,
        ),
      );
      setRows(json.data ?? []);
    } catch (err) {
      setRows([]);
      setError(errorMessage(err));
    } finally {
      setLoading(false);
    }
  }, [filter, ready]);

  useEffect(() => {
    void load();
  }, [load]);

  return { rows, loading: loading || !ready, error, reload: load };
}

export function useAgroMarketsList() {
  const { filter } = useAdminFilter();
  const { ready } = useModuleLoader();
  const [rows, setRows] = useState<AgroMarketRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!ready) return;
    setLoading(true);
    setError(null);
    try {
      const json = await fetchWithRetry(() =>
        apiClient<{ success: boolean; data: AgroMarketRow[] }>(
          `agro-markets${unitQuery(filter)}`,
        ),
      );
      setRows(json.data ?? []);
    } catch (err) {
      setRows([]);
      setError(errorMessage(err));
    } finally {
      setLoading(false);
    }
  }, [filter, ready]);

  useEffect(() => {
    void load();
  }, [load]);

  return { rows, loading: loading || !ready, error, reload: load };
}

export function useKpiData() {
  const { filter } = useAdminFilter();
  const { ready } = useModuleLoader();
  const [definitions, setDefinitions] = useState<KpiDefinition[]>([]);
  const [records, setRecords] = useState<KpiRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!ready) return;
    setLoading(true);
    setError(null);

    const unit = activeUnitId(filter);
    const recordsParams = new URLSearchParams({ limit: "200" });
    if (unit) recordsParams.set("unitId", unit);
    const recordsPath = `kpis/records?${recordsParams}`;

    const errors: string[] = [];

    const [defsResult, recsResult] = await Promise.allSettled([
      fetchWithRetry(() =>
        apiClient<{ success: boolean; data: KpiDefinition[] }>("kpis/definitions"),
      ),
      fetchWithRetry(() =>
        apiClient<{ success: boolean; data: KpiRecord[] }>(recordsPath),
      ),
    ]);

    if (defsResult.status === "fulfilled") {
      setDefinitions(defsResult.value.data ?? []);
    } else {
      setDefinitions([]);
      errors.push(`Definitions: ${errorMessage(defsResult.reason)}`);
    }

    if (recsResult.status === "fulfilled") {
      setRecords(recsResult.value.data ?? []);
    } else {
      setRecords([]);
      errors.push(`Records: ${errorMessage(recsResult.reason)}`);
    }

    setError(errors.length > 0 ? errors.join(" · ") : null);
    setLoading(false);
  }, [filter, ready]);

  useEffect(() => {
    void load();
  }, [load]);

  return { definitions, records, loading: loading || !ready, error, reload: load };
}
