...
const envSchema = z.object({
  ...
  // Heavy external fetch jobs — 15 min (balanced freshness vs VPS load)
  PIPELINE_NEWS_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_COMMODITY_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_KPI_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_ALERT_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_AGRO_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_HAZARD_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_WEATHER_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_UNREST_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_NARRATIVE_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_OUTLOOK_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  PIPELINE_BRIEFING_INTERVAL_MS: z.coerce.number().int().positive().default(900_000),
  /** Always-on national + local DB pulse (independent of browser tabs). Keep 2–5 min. */
  PIPELINE_PULSE_INTERVAL_MS: z.coerce.number().int().positive().default(300_000),
